...
<?php 
	...
	if(!empty($num_id)){
		...
		$statement=$cnx->prepare('SELECT * FROM paises WHERE num_id=:num_id ');
	...
	if(!empty($edicion)){
		...
		$statement=$cnx->prepare('UPDATE paises SET str_nombre=:str_nombre WHERE num_id=:num_id');
		$statement->execute(array(
			':num_id' => $num_id_edicion, 
			':str_nombre' => $str_nombre_edicion
			));	

			header('Location: paises_listado.php');
		...
		<div class='container' style="border:1px solid silver">
			...
			<div class='row mb-2 mt-2'>
				<div class='col text-left'>
					...
					<a href="paises_listado.php" class="btn btn-secondary ml-2" ><i class="fas fa-long-arrow-alt-left"></i> Volver </a>
